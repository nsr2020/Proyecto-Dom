...
const createSelectModel = () => {
  ...
  selectModel.classList.add("selectclass")
...
/* const createSelectStar = () => {
  const divFiltros1 = document.querySelector("#filtros");

  const selectModel1 = document.createElement("select");
  selectModel1.classList.add("selectclass")
  ...
  for (const star of ESTRELLAS) {
      ...
      option1.classList.add("optionclass")
    
      selectModel.appendChild(option1);
      
  }

  divFiltros1.appendChild(createSelectStar);

  selectModel1.addEventListener("change", (event) => {
      ESTRELLAS = event.target.value;
      filtrar();
  })
